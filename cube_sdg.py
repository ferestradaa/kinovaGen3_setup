...
parser.add_argument("--headless", type=bool, default=True, help="Launch script headless, default is False")
...
def update_semantics(stage, keep_semantics=[]):
    """ Remove semantics from the stage except for keep_semantic cslasses"""
    ...
    with rep.trigger.on_frame(num_frames=CONFIG["num_frames"]):
        ...
        if args.distractors != "None":
            with rep_distractor_group:
                rep.modify.pose(position=rep.distribution.uniform((-10, -10, 0), (10, 10, 0)),
        ...
        with rep.get.prims(path_pattern="RectLight"):
            ...
            rep.modify.attribute("intensity", rep.distribution.normal(400000.0, 600000.0))
