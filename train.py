...
def _runnetwork(net, optimizer, local_rank, epoch, train_loader, writer=None):
    ...
    for batch_idx, targets in enumerate(train_loader):
        ...
        # Añade estos chequeos antes del forward
        if not torch.isfinite(data).all():
            raise ValueError(f"NaN/Inf en input image tensor. Batch {batch_idx}, Rank {local_rank}")
        if not torch.isfinite(target_belief).all():
            raise ValueError(f"NaN/Inf en target belief. Batch {batch_idx}, Rank {local_rank}")
        if not torch.isfinite(target_affinities).all():
            raise ValueError(f"NaN/Inf en target affinities. Batch {batch_idx}, Rank {local_rank}")

        # Aquí sí haces el forward
...
def main(opt):
    ...
    # Data Augmentation
    img_w, img_h = opt.imagesize # PIL usa (alto, ancho)
    print(img_w, img_h)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        #default=448,
        nargs=2,
        default=[1280, 720], #se cambia el argumento para aceptar imagenes rectangulares 
    ...
    print(f"Training for {opt.epochs} epochs")  
